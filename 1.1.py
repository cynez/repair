...
#			login().cek_kukis()
		...
		tampilan_data = f"""{WOR} Name      : {PP}{nama}{QQ}
{WOR} Your ID   : {PP}{id}{QQ}
{WOR} Premium   : {PP}Yes
{WOR} Birthday  : {PP}{bd}
{WOR} Login Date: {PP}{datetime.now().strftime('%A %d-%m-%Y')}""";api = requests.get('http://ip-api.com/json').json();time = f"""{WOR} iP        : {PP}{api['query']}
{WOR} Country   : {PP}{api['country']}
{WOR} City      : {PP}{api['city']}
{WOR} Isp       : {PP}{api['isp']}
{WOR} Time      : {PP}{datetime.now().strftime('%H:%M')}"""
		_tomas.append(Panel(tampilan_data,width=42,title=f"{GOD}{PP}Account Information{GOD}",style=f"{A}"))
		_tomas.append(Panel(time,width=45,title=f"{GOD}{PP}Status Information{GOD}",style=f"{A}"))
